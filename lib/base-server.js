...
(() => {
  ...
  const ReverseProxy = global.use_reverse_proxy;
  ...
  class BaseServer {
    ...
    listen() {
      ...
      if (ReverseProxy) {
        return Promise.resolve()
        .then(() => this._createReverseProxyServer())
      }
      return Promise.resolve()
    ...
    _createReverseProxyServer() {
      return new Promise((resolve, reject) => {
        ...
        // set up base app
        root.set('trust proxy', 'loopback');
        root.use(helmet());

        // mount proxy
        root.use(ReverseProxy, proxy);

        // last resort
        root.use(function(req, res) {
          console.log("invalide request: ", req.url);
        });

        this.server = http.createServer(root);
        this.server.once('error', reject);
        this.server.listen(HTTP_PORT, 'localhost', () => resolve(this.server));
      });
    }
