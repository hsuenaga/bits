...
(() => {
  ...
  const TrustProxy = global.network_options.trust_proxy;
  ...
  class BaseServer {
    constructor(authManager, keyManager, certManager) {
      ...
      this.app = express();
      TrustProxy && this.app.set('trust proxy', 'loopback');
      this.app.use(helmet());
    ...
    listen() {
      ...
      if (TrustProxy) {
        logger.debug("Trust Reverse Proxy. TLS is NOT used.");
        return Promise.resolve()
        .then(() => this._createHttpTrustedServer());
      }

      return Promise.resolve()
      .then(() => this._createHttpRedirectServer())
    ...
    _createHttpTrustedServer() {
      const rootServer = express();
      const appRouter = express.Router();

      rootServer.set('trust proxy', 'loopback');
      rootServer.use(helmet());
      rootServer.use(global.network_options.root_path, appRouter);
      rootServer.use((req, res, next) => {
        console.debug('Invalid URL: ', req.url);
      });
      appRouter.use(this.app);
      appRouter.use((req, res, next) => {
        console.debug('Unhandled URL: ', req.url);
      });
      return new Promise((resolve, reject) => {
        this.server = http.createServer(rootServer);
        this.server.once('error', reject);
        this.server.listen(HTTP_PORT, () => resolve(this.server));
      });
    }

    _createHttpRedirectServer() {
      return new Promise((resolve, reject) => {
        const app = express();
        app.use(helmet());
